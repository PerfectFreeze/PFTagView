...
- (void)addTagWithText:(NSString *)text count:(NSUInteger)count {
    CGFloat fontSize = self.minFontSize + (self.maxFontSize - self.minFontSize) * (count - [self.counts[self.counts.count - 1] unsignedIntegerValue]) / ([self.counts[0] unsignedIntegerValue] - [self.counts[self.counts.count - 1] unsignedIntegerValue]);
    CGFloat width = [PFTagView widthWithText:text
                                       count:count
                                    fontSize:fontSize
    ...
    if (self.x + self.margin + width > self.frame.size.width - self.margin) {
        ...
        self.maxWidth = self.frame.size.width - self.margin;
        width = [PFTagView widthWithText:text
                                   count:count
                                fontSize:fontSize
                                maxWidth:self.maxWidth
                               maxHeight:self.configuration.tagViewHeight
    ...
    PFTagView *view = [[PFTagView alloc] initWithFrame:CGRectMake(self.x + self.margin, self.y + self.margin, width, self.configuration.tagViewHeight)
                                                  text:text
                                                 count:count
                                              fontSize:fontSize
